    ...
    <el-form :model="prop" ref="propForm" label-position="left" size="mini">
      <el-collapse v-model="activeName" accordion>
        <el-collapse-item title="组件属性" name="1">
          ...
          <el-form-item label="位置：" label-width="135px">
            <el-radio-group v-model="prop.pos" size="mini" @change="changevalue('pos')">
              <el-radio-button label="horizontal">横向</el-radio-button>
              <el-radio-button label="vertical">纵向</el-radio-button>
            </el-radio-group>
          </el-form-item>
          <el-form-item label="线条宽度：" label-width="110px">
            <el-slider v-model="prop.width" :min="1" :max="30" @change="changevalue('width')"></el-slider>
          </el-form-item>
  ...
  methods: {
    init(){
        ...
        if(this.comp.style){
          ...
          this.prop.pos = null;
          this.prop.width = null;
          this.prop.opacity = 100;
        }
    },
    compRender(){
    ...
    changevalue(col){
      ...
      if(col === 'title'){
        ...
        bsUtils.writeOperLogs(this.userOpers, {comp:this.comp, oper:"line", options:{col:col, val:s[col], ts: this}});
        s[col] = val;
      }
      this.compRender();
      this.setUpdate();
