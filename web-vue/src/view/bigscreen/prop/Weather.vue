        ...
        <el-collapse-item title="组件属性" name="1">
          ...
          <el-form-item label="字体大小：" label-width="110px">
            ...
            <el-form-item label="城市：" label-width="100px">
               <el-input v-model="prop.city" @change="changevalue('city')" placeholder="城市名称 如: 海淀,青岛,大连 (不要带市和区) 默认自适应所在城市"></el-input>
            </el-form-item>
            <el-form-item label="显示：" label-width="80px">
               <el-checkbox-group v-model="prop.showType"  @change="changevalue('showType')" >
                <el-checkbox-button label="city">城市</el-checkbox-button>
                <el-checkbox-button label="weather">天气</el-checkbox-button>
  ...
  data(){
    return {
      ...
      prop:{
         ...
         family:null,
         textAlign: null,
         showType:['city', 'weather', 'temperature'],
         city: null,
      },
      opts:{
  ...
  methods: {
    init(){
        ...
        if(this.comp.style){
          ...
          this.prop.showType = ['city', 'weather', 'temperature'];
          this.prop.city = null;
        }
    },
    compRender(){
    ...
    changevalue(col){
      ...
      if(col === 'title'){
        ...
        bsUtils.writeOperLogs(this.userOpers, {comp:this.comp, oper:"weather", options:{col:col, val:s[col], ts: this}});
        s[col] = val;
        if(col === 'city'){  //刷新天气
          var o = this.$parent.$parent;
          o.$refs['optarea'].$refs['mv_' + this.comp.id].view();
        }
      }
      this.compRender();
      this.setUpdate();
